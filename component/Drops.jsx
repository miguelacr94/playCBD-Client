import React from 'react'

const Drops = () => {
    return (
        <div>
            <h1 className="sm:mt-20 mt-9 text-black font-bold text-center">
                GOTAS CBD
            </h1>
            <div className="mt-14 w-full sm:w-4/6 h-44 text-black bg-grey m-auto">
                imagen gotas
            </div>
            <div className="w-contentResponsive  sm:w-5/6 m-auto sm:p-1 md:w-5/6">
                <p className="text-grey-Dark text-textResponsive sm:text-text xs:text-text text-center 
                m-auto font-normal font-medium font-sans mt-4 leading-4 sm:leading-6">
                    Nuestras gotas clásicas de CBD son la perfecta elección para una vida cotidiana y saludable,
                    asimismo incorpora una variedad más amplia de compuestos activos de la planta y cannabinoides.
                    Mantener alejado del calor y de la luz natural.
                </p>
            </div>

        </div>
    )
}

export default Drops